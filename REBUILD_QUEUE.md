...
10. **greatexpect-pip** — PARTIAL (2026-07-22, updated 2026-07-27): Satis House rebuilt as the gentleman-vs-blacksmith class-ladder chapter (STATUS/LOYALTY stats pick one of three endings). Down the Thames rebuilt as a branching river-escape (QUIET-vs-BOLD route choice each of 4 reaches, a COIN/SUSPICION resource trade-off, suspicion cap ends the run). The Confrontation (wharf) rebuilt as a reactive grip-read duel (telegraphed LEFT/CENTER/RIGHT brace, shrinking reaction ring, 5 grapples to win) — no longer move-dodge (2026-07-27). Only 1/4 remaining chapter is still move-dodge/collect-and-avoid: **marshes** (free-move stealth, collect 6 provisions while dodging lantern cones) — give it the same "money/status stats, branching trek" treatment next.
...
21. **grimm-tales** — an anthology, but ch2 & ch5 repeat → make each tale a *different* genre (Hansel maze, Rumpelstiltskin name-guess word puzzle, Rapunzel vertical climber).
22. **alibaba-cave** — reuses the same whack-a-mole twice → a **hide-and-seek deduction** ("which of 40 jars hides a thief?", Clue-style).
