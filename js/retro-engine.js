...
(function (global) {
  ...
  /* ---------------------- NES palette (Gen-3 authenticity) --------------- */
  // The classic NES (2C02) palette. Gen-3 (8-bit) games should pick colors
  // from here — and use FLAT fills + dithering for shades, no smooth gradients
  // or alpha glows — so the art reads as period-honest NES, not "modern retro".
  const NES = [
  ...
  const _NESRGB = NES.map(_rgb);
  // snap any hex to the nearest NES color (authoring aid for period-honest art)
  function snapNES(hex) {
    const c = _rgb(hex); let best = 0, bd = 1e9;
    for (let i = 0; i < _NESRGB.length; i++) { const p = _NESRGB[i], d = (p[0] - c[0]) ** 2 + (p[1] - c[1]) ** 2 + (p[2] - c[2]) ** 2; if (d < bd) { bd = d; best = i; } }
    return NES[best];
  }
